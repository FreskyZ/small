...
# step 3: download synthesis data
def download_synthesis_recipes():
...
# step 5: minimize 2 files to use in website, before: 571kb + 298kb, after: 23kb + 95kb
def minimize():
    with open('all.json') as f:
        operators = json.loads(f.read())
    with open('craft.json') as f:
    ...
    min_recipes = {} # { target id: set[(ingredient id 1, ingredient id 2)]}
    for from1, from2, target in recipes:
        if target not in min_recipes:
            min_recipes[target] = set()
        min_recipes[target].add((from1, from2) if from1 <= from2 else (from2, from1))

    with open('operators.json', 'w') as f:
        json.dump(min_operators, f, ensure_ascii=False, separators=(',', ':'))
    with open('recipes.json', 'w') as f:
        json.dump([{ 'target': target, 'recipes': list(ingredients) } for target, ingredients in min_recipes.items()], f, separators=(',', ':'))
