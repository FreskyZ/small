...
// END LIBRARY c276d196ff958c3d0b7ee8065b80232e9d2a3002b9a3f596a2f7d765ee7afde6
...
async function build(ecx: MessengerContext, options: CodeGenerationOptions) {
    ...
    if (ecx) {
        ...
        if (uploadResults.some(r => !r || r.status == 'error')) {
            ...
            if (uploadResults.some(r => r.path.startsWith('static/yala') && r.status == 'ok')) {
                reloadRequests.push({ kind: 'static-content:reload', key: 'yala' });
            }
            if (uploadResults.some(r => r.path.startsWith('servers/yala') && r.status == 'ok')) {
                reloadRequests.push({ kind: 'actions-server:reload', name: 'yala' });
            }
