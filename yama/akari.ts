...
`.replaceAll('api.example.com/example', `api.example.com/${config.appname}`);
    ...
    for (const action of config.actions.filter(a => a.key == 'main')) {
        ...
        if (action.parameters.length) {
            sb += `{ `;
            for (const parameter of action.parameters) {
                sb += `${parameter.name.endsWith('?') ? parameter.name.substring(0, parameter.name.length - 1) : parameter.name}, `;
            }
            sb = sb.substring(0, sb.length - 2);
            sb += ` }, `;
        } else if (action.body) {
...
// END LIBRARY afde01b9f29c7cffd9e908e1c3a125b93850883fe06600732db246761b582d25
