...
def get_info_entropy(combinations, index, debug):
    ...
    return f'{reductions / (len(combinations) - 1):.2f}'
    ...
    while True:
        ...
        if not onetry or onetry == 'help':
            ...
            if len(combinations) > 32 or len(combinations) == 1:
                print('advanced recommendation not available for over 32 possiblities')
            else:
                for i, a in enumerate(combinations):
                    entropy = get_info_entropy(combinations, i, False)
                    print(f'[{i + 1}] ' + ' '.join(NAMES[c] for c in a) + f' {entropy}')
            ...
            for guess, counts in guesses:
                print(f'[{counts[0]} HIT {counts[1]} BLOW] ' + ' '.join(NAMES[c] for c in guess))
            ...
            guesses.append((guess, counts))
            combinations = [c for c in combinations if get_count(c, guess) == counts]
            if counts[0] == 4:
                print(f'{counts[0]} HIT {counts[1]} BLOW GAME OVER!')
                break
            else:
                print(f'{counts[0]} HIT {counts[1]} BLOW')
