...
interface WorkMetadata {
    ...
    // empty for no subtitle, available values vtt, lrc, vss, txt
...
async function handleMigrateCommand(parameters: string[]) {
    ...
    // use workid vss-version: use auto generated subtitle with vss version, copies file and sets metadata
        ...
        for (const track of metadata.tracks) {
            ...
            track.subtitleProviderPath = '書き取り';
        }
    ...
    // migrate existing comments
        ...
        for (const workId of roughWorkIds) {
            ...
            // 1. inspect
            // if ((metadata as any).comment) {
            //     logInfo(`${workId}: has comment`);
            // }
            // if ((metadata as any).managementComment) {
            //     logInfo(`${workId}: has management comment`);
            // }
            // for (const track of metadata.tracks) {
            //     if ((track as any).comment) {
            //         logInfo(`${workId}: track ${track.index} has comment`);
            //     }
            // }
            // 2. manually check every work
            // 3. remove empty managementComment
            // if (typeof (metadata as any).managementComment == 'string' && (metadata as any).managementComment.length == 0) {
            //     delete (metadata as any).managementComment;
            //     await fs.writeFile(metadataPath, JSON.stringify(metadata, undefined, 2));
            // }
            // 4. manually check again, pass migrate validate
            // 5. inspect again
            if (metadata.comments && metadata.comments.length) {
                for (const comment of metadata.comments) {
                    console.log(`${workId}: ${comment}`);
                }
            }
            if (metadata.managementComments && metadata.managementComments.length) {
                for (const comment of metadata.managementComments) {
                    console.log(`${workId}: ${comment}`);
                }
            }
            for (const track of metadata.tracks) {
                if (track.comments) {
                    for (const comment of track.comments) {
                        console.log(`${workId}: track ${track.index} ${comment}`);
                    }
                }
            }
        }
